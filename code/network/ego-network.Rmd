...
autores_summary <- autores_peso %>% 
  dplyr::left_join(autores_peso, by = c("id_proposition")) %>% 
  dplyr::filter(id_deputy.x != id_deputy.y) %>% 
  dplyr::rowwise() %>% ## camada para remover direção (rows com duplicação)
  dplyr::mutate(source = min(id_deputy.x, id_deputy.y),
        ...
        a <- graph %>%
          ...
          mutate(
            brokerage_l1 = tidygraph::map_local(
              order = 1,
              mindist = 1,
              .f = function(neighborhood, graph, node, ..1 = party1, ..2 = node1, ..3 = vizinhos) {
                print(node)
                # print(neighborhood %>% activate(nodes))
                # print(paste("node1", node1, "-node2", node))
...
brokerage(g2, cl=get.vertex.attribute(g2, "party_name"))$raw.nli
```
